...
def validate_type(column: str, value: Any, expected_type: str) -> ValidationError | None:
    ...
    type_validators = {
        ...
        "string": lambda v: isinstance(v, (str, int, float)) or isinstance(v, bool) == False,  # Coerce numbers to string
        "bool": lambda v: isinstance(v, bool) or (isinstance(v, str) and v.lower() in ("true", "false", "1", "0", "yes", "no")) or isinstance(v, int),
...
def validate_range(column: str, value: Any, min_val: Any = None, max_val: Any = None) -> ValidationError | None:
...
def validate_rows(rows: list[dict[str, Any]], column_specs: dict[str, dict]) -> tuple[list[dict], list[dict]]:
    """
