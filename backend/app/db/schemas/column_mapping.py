...
class ColumnMappingCreate(BaseModel):
    ...
    source_field: str = Field(..., description="API response field name (supports dot notation for nested fields)")
    dest_column: str = Field(..., description="Oracle database column name")
    transform_rules: Optional[list[str]] = Field(None, description="List of transforms to apply: trim, upper, lower, to_int, to_float, to_bool, to_timestamp, to_date, format_date")
...
class FieldPreview(BaseModel):
    ...
    field_name: str = Field(..., description="Full dot notation field name (e.g., user.address.city)")
    field_type: str = Field(..., description="Inferred type: string, number, boolean, null, array, object")
...
class PreviewFieldsRequest(BaseModel):
    ...
    use_auto_fetch: bool = Field(False, description="Whether to auto-fetch from API or use manual sample_json")
    sample_json: Optional[dict] = Field(None, description="Manual sample JSON (required if use_auto_fetch=False)")
    method: str = Field("GET", description="HTTP method for auto-fetch")
    url: Optional[str] = Field(None, description="API URL for auto-fetch (required if use_auto_fetch=True)")
...
class FieldsPreviewResponse(BaseModel):
    ...
    sample_response: dict = Field(..., description="The raw sample response (for tree view construction)")
...
class OracleColumn(BaseModel):
    ...
    data_type: str = Field(..., description="Oracle data type (e.g., VARCHAR2, NUMBER, DATE, TIMESTAMP)")
...
class TransformSuggestionsResponse(BaseModel):
    ...
    suggestions: list[TransformSuggestion] = Field(..., description="List of recommended transforms")
    requires_transform: bool = Field(..., description="Whether a transform is necessary for compatibility")
...
class MappingTemplate(BaseModel):
    ...
    mappings: list[ColumnMappingCreate] = Field(..., description="List of mappings in this template")
