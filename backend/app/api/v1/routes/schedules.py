...
def create_schedule(
    ...
    existing_schedule = db.query(TaskSchedule).filter(
    ...
    try:
        ...
        logger.info(f"Created schedule {schedule.id} for task {task_id} with cron: {payload.cron_expression}")
...
def get_schedule(
    ...
    if not schedule:
        raise HTTPException(
            status_code=404,
            detail=f"No schedule found for task {task_id}"
        )
...
def list_schedules(
    ...
    schedules_with_names = query.order_by(
        desc(TaskSchedule.created_at)
