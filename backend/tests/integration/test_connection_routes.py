...
def client(temp_connections_file):
    ...
    import app.services.connection_file as cs
...
class TestConnectionRoutes:
    ...
    def cleanup_connections_file(self):
        ...
        # Try importing the correct connection storage service module
        try:
            from app.services.connection_storage import ConnectionStorageService
        except ImportError:
            # Fallback: try alternative import if the file/module was renamed
            from app.services.connection_storage import ConnectionStorageService

        service = ConnectionStorageService()
        if hasattr(service, "file_path") and service.file_path.exists():
