...
def create_mappings(
    ...
    for mapping_data in payload.mappings:
        ...
        mapping = ColumnMapping(
            ...
            transform_rules=json.dumps(mapping_data.transform_rules) if mapping_data.transform_rules else None,
...
async def preview_fields(
    ...
    use_auto_fetch: bool = Query(False, description="Whether to auto-fetch from API or use manual sample_json"),
    db: Session = Depends(get_db)
):
    """
    ...
    try:
        if use_auto_fetch:
            ...
            if not sample_json:
                raise HTTPException(status_code=400, detail="sample_json is required when use_auto_fetch=False")
        ...
        raise HTTPException(status_code=400, detail=f"Failed to fetch/parse sample response: {str(e)}")
...
async def preview_fields_standalone(request: PreviewFieldsRequest):
    ...
    try:
        if request.use_auto_fetch:
            if not request.url:
                raise HTTPException(status_code=400, detail="url parameter required for auto-fetch mode")
        ...
        for field_name, field_info in flattened_data.items():
            fields_info.append(FieldPreview(
...
def suggest_transforms_endpoint(
    source_type: str = Query(..., description="Source field type (string, number, boolean, array, object, null)"),
    dest_type: str = Query(..., description="Destination column type (VARCHAR2, NUMBER, DATE, TIMESTAMP, etc)")
):
    """
    ...
    try:
        ...
        logger.info(f"Generated {len(suggestions.suggestions)} suggestions for {source_type} → {dest_type}")
