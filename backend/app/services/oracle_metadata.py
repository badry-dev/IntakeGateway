...
def get_table_columns(db: Session, table_name: str) -> list[OracleColumn]:
    ...
    try:
        ...
        if "ORA-00942" in error_msg:  # Table or view does not exist
            ...
            return []
        elif "ORA-00904" in error_msg or "ORA-01031" in error_msg:  # Invalid column or insufficient privileges
            ...
            logger.error(f"Database connection error (requires Oracle Instant Client for thick mode): {error_msg}")
...
def get_column_info(db: Session, table_name: str, column_name: str) -> OracleColumn | None:
    ...
    try:
        ...
        result = db.execute(
            query,
            {"table_name": table_name_upper, "column_name": column_name_upper}
        )
        row = result.fetchone()
